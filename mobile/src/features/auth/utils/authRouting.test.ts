 ...
 * This helper ensures all tests have valid inputs after onboarding fields were added.
 *
 * Default values:
 * - hasOnboarded: true (most tests assume user already onboarded)
 * - onboardingGateEnabled: false (gate disabled by default for backward compatibility)
 */
function createAuthInput(
  partial: Pick<AuthRoutingInput, 'isAuthenticated' | 'isVerified'> &
    Partial<Pick<AuthRoutingInput, 'hasOnboarded' | 'onboardingGateEnabled'>>
): AuthRoutingInput {
  return {
    hasOnboarded: true,
...
describe('deriveInitialRouteFromAuthState', () => {
  ...
  describe('Function properties', () => {
    ...
    it('should be deterministic - same values produce same result', () => {
      const input1 = createAuthInput({
        isAuthenticated: true,
        isVerified: true,
      });

      const input2 = createAuthInput({
        isAuthenticated: true,
        isVerified: true,
      });
  ...
  describe('Complete coverage matrix', () => {
    it('should handle all four possible boolean combinations correctly', () => {
      ...
      const testCases: {
        ...
        {
          input: createAuthInput({ isAuthenticated: false, isVerified: false }),
          expected: 'login',
          description: 'unauthenticated and unverified',
        },
        {
          input: createAuthInput({ isAuthenticated: false, isVerified: true }),
          expected: 'login',
          description: 'unauthenticated but verified (impossible state)',
        },
        {
          input: createAuthInput({ isAuthenticated: true, isVerified: false }),
          expected: 'verify',
          description: 'authenticated but unverified',
        },
        {
          input: createAuthInput({ isAuthenticated: true, isVerified: true }),
          expected: 'home',
  ...
  describe('Security model', () => {
    it('should implement fail-safe defaults by routing to login for unauthenticated users', () => {
      ...
      const unauthenticatedInputs: AuthRoutingInput[] = [
        createAuthInput({ isAuthenticated: false, isVerified: false }),
        createAuthInput({ isAuthenticated: false, isVerified: true }),
      ];
    ...
    it('should require explicit verification flag for home access', () => {
      ...
      const deniedInputs: AuthRoutingInput[] = [
        createAuthInput({ isAuthenticated: false, isVerified: false }),
        createAuthInput({ isAuthenticated: false, isVerified: true }),
        createAuthInput({ isAuthenticated: true, isVerified: false }),
      ];
      ...
      const allowedInput = createAuthInput({
        isAuthenticated: true,
        isVerified: true,
      });
  ...
  describe('Route protection validation scenarios', () => {
    it('should deny home access without authentication', () => {
      ...
      const unauthenticated = createAuthInput({
        isAuthenticated: false,
        isVerified: false,
      });
      ...
      const unauthButVerified = createAuthInput({
        isAuthenticated: false,
        isVerified: true,
      });
  ...
  describe('Idempotency across scenarios', () => {
    it('should produce same route for same state regardless of how state was reached', () => {
      ...
      const loginPath = createAuthInput({
        isAuthenticated: true,
        isVerified: true,
      });

      const restorePath = createAuthInput({
        isAuthenticated: true,
        isVerified: true,
      });

      const verificationPath = createAuthInput({
        isAuthenticated: true,
        isVerified: true,
      });

      const backgroundPath = createAuthInput({
        isAuthenticated: true,
        isVerified: true,
      });
    ...
    it('should have no state leakage between different scenario calls', () => {
      ...
      const inputs: AuthRoutingInput[] = [
        createAuthInput({ isAuthenticated: false, isVerified: false }),
        createAuthInput({ isAuthenticated: true, isVerified: false }),
        createAuthInput({ isAuthenticated: true, isVerified: true }),
        createAuthInput({ isAuthenticated: false, isVerified: true }),
      ];
  ...
  describe('Comprehensive scenario coverage matrix', () => {
    ...
    it('should serve as executable specification for all auth routing decisions', () => {
      ...
      completeMatrix.forEach(({ isAuthenticated, isVerified, expectedRoute }) => {
        const input = createAuthInput({ isAuthenticated, isVerified });
        const result = deriveInitialRouteFromAuthState(input);
